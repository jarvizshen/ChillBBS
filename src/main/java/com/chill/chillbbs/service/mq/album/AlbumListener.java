...
public class AlbumListener {
    ...
    @RabbitListener(queues = {Constants.DECREASE_ALBUM_COMMENT_NUMBER_QUEUE})
    public void decreaseCollectListener(Long id, Channel channel,
                                        @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        try {
            log.info("减少收藏收到参数：{}", id);
            albumService.decreaseCollect(id);
            channel.basicAck(tag, false);
        } catch (Exception e) {
            try {
                //nack返回false，重新返回队列
    ...
    @RabbitListener(queues = {Constants.INCREASE_ALBUM_COMMENT_NUMBER_QUEUE})
    public void increaseCollectListener(Long id, Channel channel,
                                        @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        try {
            log.info("增加收藏收到参数：{}", id);
            albumService.increaseCollect(id);
            channel.basicAck(tag, false);
        } catch (Exception e) {
            try {
                //nack返回false，重新返回队列
